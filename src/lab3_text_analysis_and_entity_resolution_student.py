...
def tokenize(string):
    ...
    """
    nonEmptyTokens = (filter(lambda x:x!='',re.split(split_regex, string.lower())))
    return [x for x in nonEmptyTokens if x not in stopwords ]
...
def countTokens(vendorRDD):
    ...
    """
    return vendorRDD.map(lambda s:len(s[1])).reduce(lambda x,y:x+y)
...
def findBiggestRecord(vendorRDD):
    ...
    """
    return vendorRDD.sortBy(lambda x:len(x[1]),False).collect()
...
#     for k in dict:
#         dict[k] = dict[k]/float(len(tokens))
    dict ={ k:dict[k]/float(len(tokens)) for k in dict}
...
def idfs(corpus):
    ...
    tokenCountPairTuple = uniqueTokens.flatMap(lambda s:[(g,1) for g in s])
    tokenSumPairTuple = tokenCountPairTuple.reduceByKey(lambda a,b:a+b)
    return (tokenSumPairTuple.map(lambda y:(y[0],N/float(y[1]))))
...
def tfidf(tokens, idfs):
    ...
    """
    tfs = {t:float(tokens.count(t))/len(tokens) for t in tokens}
    tfIdfDict = {tok:tfs[tok]*idfs[tok] for tok in tokens}
    return tfIdfDict
...
trueDupsRDD = (sims.join(goldStandard).map(lambda x:(x[0],x[1][0])))
               
trueDupsCount = trueDupsRDD.distinct().count()
avgSimDups = trueDupsRDD.map(lambda c:c[1]).reduce(lambda a,b:a+b)/float(trueDupsCount)
...
amazonWeightsRDD = amazonFullRecToToken.map(lambda y:(y[0], tfidf(y[1], idfsFullBroadcast.value)))
googleWeightsRDD = googleFullRecToToken.map(lambda y:(y[0], tfidf(y[1], idfsFullBroadcast.value)))
...
# TODO: Replace <FILL IN> with appropriate code
amazonNorms = amazonWeightsRDD.mapValues(lambda f:norm(f)).collectAsMap()
amazonNormsBroadcast = sc.broadcast(amazonNorms)
googleNorms = googleWeightsRDD.mapValues(lambda f:norm(f)).collectAsMap()
...
def invert(record):
    ...
    """
    pairs =[(r,record[0]) for r in record[1].keys()]
    return (pairs)
