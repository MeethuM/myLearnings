...
def parsePoint(line):
    ...
    """
    sp = line.split(",")
    return LabeledPoint(sp[0],sp[1:])

parsedSamplePoints = sc.parallelize(samplePoints).map(parsePoint)
...
# TODO: Replace <FILL IN> with appropriate code
parsedData = parsedDataInit.map(lambda s:LabeledPoint(s.label-1922,s.features))
...
# TODO: Replace <FILL IN> with appropriate code
averageTrainYear = (parsedTrainData.map(lambda x:x.label)
                    .mean())
...
def calcRMSE(labelsAndPreds):
    ...
    """
    return math.sqrt((labelsAndPreds.map(lambda r:squaredError(r[0],r[1]))
...
# TODO: Replace <FILL IN> with appropriate code
labelsAndPredsTrain = parsedTrainData.map(lambda f:(f.label,averageTrainYear))
rmseTrainBase = calcRMSE(labelsAndPredsTrain)

labelsAndPredsVal = parsedValData.map(lambda f:(f.label,averageTrainYear))
rmseValBase = calcRMSE(labelsAndPredsVal)

labelsAndPredsTest = parsedTestData.map(lambda f:(f.label,averageTrainYear))
...
def linregGradientDescent(trainData, numIters):
    ...
    for i in range(numIters):
        ...
        labelsAndPredsTrain = trainData.map(lambda f:getLabeledPrediction(w,f))
        errorTrain[i] = calcRMSE(labelsAndPredsTrain)

        # Calculate the `gradient`.  Make use of the `gradientSummand` function you wrote in (3a).
        # Note that `gradient` sould be a `DenseVector` of length `d`.
        gradient = trainData.map(lambda f:gradientSummand(w,f)).sum()
...
# TODO: Replace <FILL IN> with appropriate code
labelsAndPreds = parsedValData.map(lambda d:(d.label,firstModel.predict(d.features)))
...
def twoWayInteractions(lp):
    ...
    """
    l = lp.features
    two_way = np.hstack((l,[i[0]*i[1]for i in list(itertools.product(l,repeat=2))]))
    return LabeledPoint(lp.label,two_way)
...
# TODO: Replace <FILL IN> with appropriate code
labelsAndPredsTest = testDataInteract.map(lambda l:(l.label,modelInteract.predict(l.features)))
