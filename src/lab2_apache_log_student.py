...
not200 = access_logs.filter(lambda f:f.response_code!=200)

endpointCountPairTuple = not200.map(lambda g:(g.endpoint,1))
...
hosts = access_logs.map(lambda r:r.host)
...
dayHostCount = dayGroupedHosts.map(lambda f:(f[0],len(f[1])))
...
avgDailyReqPerHost = (sortedByDay.
                      mapValues(lambda z:len(z[0])/z[1])).cache()
...
badEndpoints = badRecords.map(lambda d:d.endpoint)
...
badEndpointsCountPairTuple = badRecords.map(lambda r:(r.endpoint,1))
...
errHostsCountPairTuple = badRecords.map(lambda f:(f.host,1))

errHostsSum = errHostsCountPairTuple.reduceByKey(lambda x,y:x+y)

errHostsTop25 = errHostsSum.takeOrdered(25,key=lambda x:-1*x[1])
...
errDateCountPairTuple = badRecords.map(lambda f:(f.date_time.day,1))
...
topErrDate = errDateSorted.takeOrdered(5, key=(lambda x:-1*x[1]))
...
hourCountPairTuple = badRecords.map(lambda r:(r.date_time.hour,1))
