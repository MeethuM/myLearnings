...
def getCountsAndAverages(IDandRatingsTuple):
    ...
    """
    print IDandRatingsTuple
    numOfRatings = len(IDandRatingsTuple[1])
    avgOfRatings=sum(IDandRatingsTuple[1])/float(numOfRatings)
    return IDandRatingsTuple[0],(numOfRatings,avgOfRatings)
...
movieIDsWithAvgRatingsRDD = movieIDsWithRatingsRDD.map(lambda x:(getCountsAndAverages(tuple(x))))
...
movieNameWithAvgRatingsRDD = (moviesRDD
                              ...
                              .map(lambda c:(c[1][1][1],c[1][0],c[1][1][0])))
...
def computeError(predictedRDD, actualRDD):
    ...
    predictedReformattedRDD = predictedRDD.map(lambda h:((h[0],h[1]),(h[2])))

    # Transform actualRDD into the tuples of the form ((UserID, MovieID), Rating)
    actualReformattedRDD = actualRDD.map(lambda h:((h[0],h[1]),(h[2])))

    # Compute the squared error for each matching entry (i.e., the same (User ID, Movie ID) in each
    # RDD) in the reformatted RDDs using RDD transformtions - do not use collect()
    squaredErrorsRDD = (predictedReformattedRDD
                        .join(actualReformattedRDD)
                        .map(lambda f:(math.pow(f[1][0]-f[1][1],2))))
...
validationForPredictRDD = validationRDD.map(lambda f:(f[0],f[1]))
...
myModel = ALS.train(trainingRDD,8, seed=5L, iterations=5,
                      lambda_=0.1)
testForPredictingRDD = testRDD.map(lambda f:(f[0],f[1]))
...
trainingAvgRating = trainingRDD.map(lambda g:g[2]).reduce(lambda g,h:g+h)/float(trainingRDD.count())
...
# TODO: Replace <FILL IN> with appropriate code
myRatingsModel = ALS.train(trainingWithMyRatingsRDD,8, seed=5L, iterations=5,
                      lambda_=0.1)
...
myUnratedMoviesRDD = (moviesRDD.flatMap(lambda x:[(myUserID,x[0])] if x[0] not in (i[1] for i in myRatedMovies) else []))
...
movieCountsRDD = movieIDsWithAvgRatingsRDD.map(lambda d:(d[0],d[1][0]))
...
ratingsWithNamesRDD = (predictedWithCountsRDD
                       .filter(lambda f:f[1][1]>75)
                       .join(moviesRDD)
                       .map(lambda y:(y[1][0][0],y[1][1],y[1][0][1])))
